...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), mSettings("Motolink", "Motolink"), mHelpViewer(NULL),
      mUndoStack(NULL), mFuelModel(&mUndoStack, -30, 30, 0),
...
void MainWindow::Quit() {
  if (mHasChanged) {
    ...
    int ret = QMessageBox::question(
        this, tr("Confirm"), tr("Unsaved changes!\nSave file before closing?"),
...
void MainWindow::openFile(void) {
  QString fileName(QFileDialog::getOpenFileName(this, tr("Open Tune File"), "",
                                                tr("Tune Files (*.xml)")));
...
void MainWindow::saveFileAs(void) {
  QString fileName(QFileDialog::getSaveFileName(this, tr("Save Tune File"), "",
                                                tr("Tune Files (*.xml)")));
...
void MainWindow::showAbout() {
  QMessageBox::information(
      this, tr("About Motolink"),
      tr("<strong>Version: ") + __MTL_VER__ + "</strong><br/>" +
          tr("Built on: ") + QString(__DATE__) + " " + QString(__TIME__) +
          "<br/><br/>" +
          tr("Motolink is a smart interface designed for Honda HRC "
             "ECUs.<br/><br/>"
             "You can find more information "
             "<a href=\"https://github.com/fpoussin/MotoLink\">here.</a>"));
}

void MainWindow::showUpdateDialog() { mUpdateWizard->showWizard(); }

void MainWindow::importHrc() {
  QString fileName(QFileDialog::getOpenFileName(this, tr("Import HRC File"), "",
                                                tr("HRC File (*.E2P)")));
  mHrc->openFile(fileName);
}

void MainWindow::exportHrc() {
  QString fileName(QFileDialog::getSaveFileName(this, tr("Export HRC File"), "",
                                                tr("HRC File (*.E2P)")));
...
void MainWindow::setupConnections(void) {
  QObject::connect(mMainUi->actionAbout, SIGNAL(triggered()), this,
                   SLOT(showAbout()));
  QObject::connect(mMainUi->actionAbout_QT, SIGNAL(triggered()), qApp,
                   SLOT(aboutQt()));
  QObject::connect(mMainUi->actionQuit, SIGNAL(triggered()), this,
                   SLOT(Quit()));
  QObject::connect(mMainUi->actionOpen, SIGNAL(triggered()), this,
                   SLOT(openFile()));
  QObject::connect(mMainUi->actionSave, SIGNAL(triggered()), this,
                   SLOT(saveFile()));
  QObject::connect(mMainUi->actionSave_As, SIGNAL(triggered()), this,
                   SLOT(saveFileAs()));
  QObject::connect(mMainUi->actionConnect, SIGNAL(triggered()), this,
                   SLOT(connectMtl()));
  QObject::connect(mMainUi->actionDisconnect, SIGNAL(triggered()), this,
                   SLOT(disconnectMtl()));
  QObject::connect(mMainUi->actionUpdate, SIGNAL(triggered()), this,
                   SLOT(showUpdateDialog()));
  QObject::connect(mMainUi->actionImport, SIGNAL(triggered()), this,
                   SLOT(importHrc()));
  QObject::connect(mMainUi->actionExport, SIGNAL(triggered()), this,
                   SLOT(exportHrc()));
  QObject::connect(mMainUi->actionEnglish, SIGNAL(triggered()), this,
                   SLOT(setLanguageEnglish()));
  QObject::connect(mMainUi->actionFran_ais, SIGNAL(triggered()), this,
                   SLOT(setLanguageFrench()));
  QObject::connect(mMainUi->actionShowHelpIndex, SIGNAL(triggered()), this,
                   SLOT(showHelp()));
  QObject::connect(mMainUi->actionShow_tasks, SIGNAL(triggered()), this,
                   SLOT(showTasks()));
  QObject::connect(mMainUi->actionShow_Knock_Spectrum, SIGNAL(triggered()),
                   this, SLOT(showKnockGraph()));
  QObject::connect(mMainUi->actionShow_Logs, SIGNAL(triggered()), this,
                   SLOT(showLogs()));
  QObject::connect(mMainUi->actionShow_Serial_Data, SIGNAL(triggered()), this,
                   SLOT(showSerialData()));

  QObject::connect(mMainUi->actionShow_actions, SIGNAL(triggered()), &mUndoView,
                   SLOT(show()));
  QObject::connect(mMainUi->actionUndo, SIGNAL(triggered()), &mUndoStack,
                   SLOT(undo()));
  QObject::connect(mMainUi->actionRedo, SIGNAL(triggered()), &mUndoStack,
                   SLOT(redo()));
  QObject::connect(&mUndoStack, SIGNAL(canRedoChanged(bool)),
                   mMainUi->actionRedo, SLOT(setEnabled(bool)));
  QObject::connect(&mUndoStack, SIGNAL(canUndoChanged(bool)),
                   mMainUi->actionUndo, SLOT(setEnabled(bool)));

  // Tables (color and cursor update)
  for (int i = 0; i < mTablesModelList.size(); i++) {
    TableModel *tbl = mTablesModelList.at(i);
    QObject::connect(tbl, SIGNAL(cellValueChanged(int, int)), this,
                     SLOT(onDataChanged()));
    QObject::connect(
        tbl, SIGNAL(headerDataNeedSync(int, Qt::Orientation, QVariant)), this,
        SLOT(onHeadersNeedSync(int, Qt::Orientation, QVariant)));
    QObject::connect(tbl->view(), SIGNAL(modelUpdated(QWidget *)),
                     mMainUi->tabMain, SLOT(setCurrentWidget(QWidget *)));
    if (tbl->id() > 0)
      QObject::connect(tbl->view(), SIGNAL(cellCleared(uint, int, int)), mMtl,
                       SLOT(clearCell(uint, int, int)));
  }

  for (int i = 0; i < mSpinBoxList.size(); i++) {
    QSpinBox *sb = mSpinBoxList.at(i);
    QObject::connect(sb, SIGNAL(valueChanged(int)), this,
                     SLOT(showSettingsTab()));
    QObject::connect(sb, SIGNAL(valueChanged(int)), this,
                     SLOT(onDataChanged()));
  }

  QObject::connect(this, SIGNAL(signalStartupComplete()), &mUpdate,
                   SLOT(getLatestVersion()));
  QObject::connect(&mUpdate, SIGNAL(newVersionAvailable(QString)), this,
                   SLOT(showNewVersionPopup(QString)));
  QObject::connect(mUpdateWizard, SIGNAL(sendDisconnect()), this,
                   SLOT(disconnectMtl()));

  QObject::connect(mMainUi->bTpsSet0, SIGNAL(clicked()), this,
                   SLOT(onSetTps0Pct()));
  QObject::connect(mMainUi->bTpsSet100, SIGNAL(clicked()), this,
                   SLOT(onSetTps100Pct()));

  QObject::connect(mMainUi->bReadMtl, SIGNAL(clicked()), this,
                   SLOT(onReadMtlSettings()));
  QObject::connect(mMainUi->bWriteMtl, SIGNAL(clicked()), this,
                   SLOT(onWriteMtlSettings()));

  // Recent files
  for (int i = 0; i < MAX_RECENT_FILES; ++i) {
    mRecentFilesActions[i] = new QAction(this);
    mRecentFilesActions[i]->setVisible(false);
    mRecentFilesActions[i]->setIcon(
        QIcon("://oxygen/32x32/actions/quickopen-file.png"));
    connect(mRecentFilesActions[i], SIGNAL(triggered()), this,
            SLOT(openRecenFile()));
  ...
  QObject::connect(&mFastPollingTimer, SIGNAL(timeout()), this,
                   SLOT(doFastPolling()));
  QObject::connect(&mSlowPollingTimer, SIGNAL(timeout()), this,
                   SLOT(doSlowPolling()));
  QObject::connect(&mTablesTimer, SIGNAL(timeout()), this,
                   SLOT(doTablesPolling()));
  QObject::connect(&mRedrawTimer, SIGNAL(timeout()), this,
                   SLOT(doSensorsRedraw()));

  QObject::connect(mMtl, SIGNAL(receivedMonitoring(TaskList const *)), this,
                   SLOT(onMonitoringReceived(TaskList const *)));
  QObject::connect(mMtl, SIGNAL(receivedKockSpectrum(QByteArray const *)), this,
                   SLOT(onKnockSpectrumReceived(QByteArray const *)));
  QObject::connect(mMtl, SIGNAL(receivedTables(const quint8 *, const quint8 *)),
                   this,
                   SLOT(onTablesReceived(const quint8 *, const quint8 *)));
  QObject::connect(mMtl, SIGNAL(communicationError(QString)), this,
                   SLOT(log(QString)));
  QObject::connect(mMtl, SIGNAL(communicationError(QString)), mMtl,
                   SLOT(clearUsb()));

  QObject::connect(&mFile, SIGNAL(readFailed(QString)), this,
                   SLOT(onSimpleError(QString)));
}
...
void MainWindow::setupTabShortcuts() {
  ...
  for (int index = 0; index < mMainUi->tabMain->count(); ++index) {
    if (index > 7)
      break;
    QShortcut *shortcut =
        new QShortcut(QKeySequence(QString("F%1").arg(index + 5)), this);

    QObject::connect(shortcut, SIGNAL(activated()), signalMapper, SLOT(map()));
    signalMapper->setMapping(shortcut, index);
  }
  QObject::connect(signalMapper, SIGNAL(mapped(int)), mMainUi->tabMain,
...
void MainWindow::setupSettings() {
  ...
  mRecentFiles =
      mSettings.value(SETTINGS_RECENT_FILES, QStringList()).toStringList();
...
void MainWindow::setupKnockGraph() {
  ...
  mKnockFreqLabel->position->setCoords(
      0.5, 0); // place position at center/top of axis rect
  mKnockFreqLabel->setText(tr("Loading..."));
  mKnockFreqLabel->setFont(
      QFont(font().family(), 16));          // make font a bit larger
...
void MainWindow::showHelp() { mHelpViewer.show(); }
...
void MainWindow::exportToMTLFile() {
  ...
  mFile.addProperty("TPS0", mMainUi->dsbTPS0->value());
  mFile.addProperty("TPS100", mMainUi->dsbTPS100->value());

  mFile.addProperty("AFR0V", mMainUi->dsbAFR0->value());
  mFile.addProperty("AFR5V", mMainUi->dsbAFR5->value());
}
...
void MainWindow::doSensorsRedraw() {
  ...
  mMainUi->lSpeedHertz->setText(QString::number(mMtl->getSpeedHz()) +
                                tr(" Hertz"));
...
void MainWindow::onHeadersNeedSync(int section, Qt::Orientation orientation,
...
void MainWindow::showNewVersionPopup(QString version) {
  QMessageBox::information(
      ...
      tr("There is a new version available for download: ") + version +
          ...
          "<br/><br/><a "
          "href='https://github.com/fpoussin/MotoLink/releases/latest'>" +
          tr("Download here") + "</a>");
}
