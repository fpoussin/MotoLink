...
MTLFile::MTLFile(QObject *parent) : QObject(parent) { mLoading = false; }
...
bool MTLFile::rmTable(const QString &name) { return mTableList.remove(name); }
...
bool MTLFile::rmProperty(const QString &name) { return mPropList.remove(name); }

bool MTLFile::isLoading() { return mLoading; }
...
bool MTLFile::write(QFile *file) {
  ...
  for (int i = 0; i < rows; i++) {
    ...
    writer.writeCharacters(
        firstTable->headerData(i, Qt::Vertical, Qt::EditRole).toString());
  ...
  for (int i = 0; i < columns; i++) {
    ...
    writer.writeCharacters(
        firstTable->headerData(i, Qt::Horizontal, Qt::EditRole).toString());
...
bool MTLFile::read(QFile *file) {
  ...
  if (properties.isNull() || tables.isNull() || rows.isNull() ||
      columns.isNull()) {
  ...
  while (!prop.isNull()) {
    mPropList.insert(prop.attribute("Name"), prop.text());
