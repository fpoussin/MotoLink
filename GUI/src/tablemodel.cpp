...
TableModel::TableModel(QUndoStack *stack, int min, int max, int def,
                       bool singlerow, bool permanent, QObject *parent)
...
bool TableModel::isPermanent() { return mPermanent; }
...
bool TableModel::getCell(uint tp, uint rpm, int *row, int *col) {
  ...
  *col = -1;
  ...
  for (int i = 0; i < maxcol; i++) {
    ...
    if (rpm >=
        this->headerData(maxcol, Qt::Horizontal, Qt::EditRole).toUInt()) {
      *col = maxcol;
      break;
...
void TableModel::setView(QEnhancedTableView *view) { mView = view; }

void TableModel::setId(uint id) { mId = id; }
...
void TableModel::setSingleRow(bool val) { mSinglerow = val; }

QEnhancedTableView *TableModel::view() { return mView; }

void TableModel::setName(const QString name) { mName = name; }

void TableModel::setMin(int min) { mMin = min; }

void TableModel::setMax(int max) { mMax = max; }
...
QWidget *NumberFormatDelegate::createEditor(QWidget *parent,
...
QString AfrFormatDelegate::displayText(const QVariant &value,
                                       const QLocale &locale) const {
  bool ok;
...
QWidget *AfrFormatDelegate::createEditor(QWidget *parent,
                                         const QStyleOptionViewItem &option,
                                         const QModelIndex &index) const {
  QWidget *editor = QStyledItemDelegate::createEditor(parent, option, index);
  QLineEdit *lineEditEditor = qobject_cast<QLineEdit *>(editor);
