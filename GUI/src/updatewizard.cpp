...
UpdateWizard::~UpdateWizard() { delete mUi; }

void UpdateWizard::showWizard() {
  this->loadDefaultFirmareData();
  QString ver("Versions: [Bootloader %1] [App %2]");
  if (mMtl->usbConnect()) {
    mUi->lVersions->setText(
        ver.arg(mMtl->getVersion(0)).arg(mMtl->getVersion(1)));
  } else {
...
void UpdateWizard::startUpdate() { mUi->pbProgress->setValue(50); }

void UpdateWizard::retranslate() { mUi->retranslateUi(this); }

void UpdateWizard::openCustomFw() {
  QString fileName(QFileDialog::getOpenFileName(
      this, tr("Open firmware file"), "", tr("Firmware files (*.bin)")));
...
void UpdateWizard::updateStatus(QString text) { mUi->tbStatus->append(text); }
...
void UpdateWizard::setupConnections() {
  /* Tranfer control */
  QObject::connect(this, SIGNAL(startTransfer(QByteArray *)), mMtl,
                   SLOT(startUpdate(QByteArray *)));
  QObject::connect(this->button(QWizard::CancelButton), SIGNAL(clicked()), mMtl,
                   SLOT(haltTransfer()));

  /* Tranfer status update */
  QObject::connect(mMtl, SIGNAL(signalStatus(QString)), this,
                   SLOT(updateStatus(QString)));
  QObject::connect(mMtl, SIGNAL(transferProgress(int)), mUi->pbProgress,
                   SLOT(setValue(int)));
  QObject::connect(mMtl, SIGNAL(updateDone()), this, SLOT(updateDone()));

  QObject::connect(mMtl, SIGNAL(connectionProgress(int)), mUi->pbProgress,
                   SLOT(setValue(int)));

  QObject::connect(this, SIGNAL(currentIdChanged(int)), this,
                   SLOT(pageUpdated(int)));
  QObject::connect(this, SIGNAL(sendStatus(QString)), this,
                   SLOT(updateStatus(QString)));

  QObject::connect(mUi->rbCustomFW, SIGNAL(clicked()), this,
                   SLOT(openCustomFw()));
  QObject::connect(mUi->rbBundledFW, SIGNAL(clicked()), this,
                   SLOT(loadDefaultFirmareData()));
}
