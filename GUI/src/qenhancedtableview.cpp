...
QEnhancedTableView::QEnhancedTableView(QWidget *parent)
    : QTableView(parent), mHeaderEditUi(new Ui::HeaderEdit),
      mCellEditUi(new Ui::CellEdit), mHeaderEditDialog(new QDialog),
      mCellEditDialog(new QDialog) {
  mHeaderEditUi->setupUi(mHeaderEditDialog);
  mHeaderEditDialog->setParent(this);
  mHeaderEditDialog->setWindowFlags(Qt::Dialog | Qt::FramelessWindowHint);
  ...
  if (mMenuReadOnly) {
    ...
    if (selectedItem == actions[0]) {
      for (int i = 0; i < selection.size(); i++) {
        emit cellCleared(model->id(), selection.at(i).row(),
    ...
    actions[0] =
        myMenu.addAction(tr("Increase by ") + QString::number(increment));
    actions[1] =
        myMenu.addAction(tr("Decrease by ") + QString::number(increment));
    ...
    actions[2]->setIcon(
        QIcon("://oxygen/32x32/actions/quickopen-function.png"));
...
void QEnhancedTableView::clickedVerticalHeader(int section) {
  ...
  const int horizontalOffset =
      (mHeaderEditDialog->height() / 2) - (sectionSize / 2);
...
void QEnhancedTableView::clickedHorizontalHeader(int section) {
  ...
  const int horizontalOffset =
      (mHeaderEditDialog->width() / 2) - (sectionSize / 2);
...
void QEnhancedTableView::setupConnections() {
  ...
  QObject::connect(this->horizontalHeader(), SIGNAL(sectionDoubleClicked(int)),
                   this, SLOT(clickedHorizontalHeader(int)));
  QObject::connect(this->verticalHeader(), SIGNAL(sectionDoubleClicked(int)),
                   this, SLOT(clickedVerticalHeader(int)));
  QObject::connect(mHeaderEditUi->buttonBox, SIGNAL(accepted()), this,
                   SLOT(applyChanges()));
}

void QEnhancedTableView::setEditBoundaries(int section,
  ...
  if (section == 0) {
    ...
    min = this->model()
              ->headerData(minSection, orientation)
              .toString()
              .remove("%")
              .toInt() +
          margin;
  ...
  max = this->model()
            ->headerData(maxSection, orientation)
            .toString()
            .remove("%")
            .toInt() -
        margin;
