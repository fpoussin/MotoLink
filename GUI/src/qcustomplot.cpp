...
/*! \overload
  ...
  unpredictable when
  antialiasing is disabled. Thus when antialiasing is disabled, it rounds the \a
  line to
...
/*! \class QCPScatterStyle
  ...
  fill (\ref setBrush), if the shape provides a fillable area. Further, the size
  of the shape can
  ...
  to QCPGraph, the line
  color of the graph (\ref QCPGraph::setPen) will be used by the scatter points.
  This makes
...
/*!
  ...
  undefined pen (\ref isPenDefined), sets the pen of \a painter to \a defaultPen
  instead.
...
/*! \class QCPLayer
  ...
  is placed on the
  "background" layer. Of course, the layer affiliation of the individual objects
  can be changed as
...
/*! \fn virtual void QCPLayerable::applyDefaultAntialiasingHint(QCPPainter
  *painter) const = 0
  ...
  only the QCPGraph::applyDefaultAntialiasingHint function (which corresponds to
  the graph line's
...
/*!
  ...
  Direct layerable parents
  are mainly used to control visibility in a hierarchy of layerables. This means
  a layerable is
  only drawn, if all its ancestor layerables are also visible. Note that \a
  parentLayerable does
  not become the QObject-parent (for memory management) of this layerable, \a
  plot does.
*/
...
/*!
  ...
  considered a hit. In
  these cases this function thus returns a constant value greater zero but still
  below the parent
  ...
  is handled by the
  parent QCustomPlot when the mouseReleaseEvent occurs, and the finally selected
  object is notified
  ...
  parent QCustomPlot
  decides on the basis of this selectTest call, that the object was successfully
  selected. The
...
/*! \internal
  ...
  parent QCustomPlot).
  The \a details data you output from \ref selectTest is fed back via \a details
  here. You may
...
/*! \internal
  ...
  non-null), must
  return true or false when the selection state of this layerable has changed or
  not changed,
  respectively.
...
/*!
  ...
  functions.
  Larger absolute values would cause errors due to the 11-bit exponent of double
  precision numbers,
...
/*!
  ...
  scales, that
  the range won't span the positive and negative sign domain, i.e. contain zero.
  Further
...
/*! \class QCPMarginGroup
  ...
  left axis of the lower axis rect, making them appear aligned. The same applies
  for the right
...
/*! \class QCPLayoutElement
  ...
  has an outer rect
  (QCPLayoutElement::outerRect) and an inner rect (\ref QCPLayoutElement::rect).
  The difference
  ...
  that margin can be
  set explicitly with \ref setMargins and will stay fixed at that value. If it's
  set to automatic,
...
QCPLayoutElement::~QCPLayoutElement() {
  ...
  if (qobject_cast<QCPLayout *>(mParentLayout)) // the qobject_cast is just a
                                                // safeguard in case the layout
                                                // forgets to call clear() in
                                                // its dtor and this dtor is
                                                // called by QObject dtor
...
/*!
  ...
  layout via \ref
  setOuterRect) and the inner \ref rect (which usually contains the main content
  of this layout
  element).
...
/*! \class QCPLayout
  ...
  \ref take, \ref
  simplify, \ref removeAt, \ref remove and \ref clear. Individual subclasses may
  add more functions
...
/*! \fn virtual QCPLayoutElement* QCPLayout::elementAt(int index) const = 0
  ...
  layouts (e.g.
  QCPLayoutGrid), so this function may return 0 in those cases. You may use this
  function to check
...
/*! \internal
  ...
  no minimum size
  imposed, set all vector values to zero. If the \a minSizes entries add up to a
  value greater than
  \a totalSize, sections will be scaled smaller than the proposed minimum sizes.
  (In other words,
...
QVector<int> QCPLayout::getSectionSizes(QVector<int> maxSizes,
  ...
  while (!unfinishedSections.isEmpty() &&
         outerIterations < sectionCount * 2) // the iteration check ist just a
                                             // failsafe in case something
                                             // really strange happens
  {
    ++outerIterations;
    int innerIterations = 0;
    while (!unfinishedSections.isEmpty() &&
           innerIterations < sectionCount * 2) // the iteration check ist just a
                                               // failsafe in case something
                                               // really strange happens
    {
      ++innerIterations;
      ...
      // section
      // actually hits its maximum, without exceeding the total size when we add
      // up all sections)
      ...
      if (nextMax < nextMaxLimit) // next maximum is actually hit, move forward
                                  // to that point and fix the size of that
                                  // section
      {
        for (int i = 0; i < unfinishedSections.size(); ++i) {
    ...
    if (foundMinimumViolation) {
      freeSize = totalSize;
      for (int i = 0; i < sectionCount; ++i) {
        if (!minimumLockedSections.contains(i)) // only put sections that
                                                // haven't hit their minimum
                                                // back into the pool
          unfinishedSections.append(i);
        else
          freeSize -= sectionSizes.at(i); // remove size of minimum locked
                                          // sections from available space in
                                          // next round
      }
...
/*! \internal
  ...
  that row. The
  maximum width of a column is the smallest maximum width of any element in that
  column.
...
/*! \class QCPLayoutInset
  ...
  addElement(QCPLayoutElement *element, const QRectF &rect). If the first method
  is used, the inset
  placement will default to \ref ipBorderAligned and the element will be aligned
  according to the
...
/*!
  ...
  rect (0, 0, 1, 1)
  will span the entire layout. An inset with rect (0.6, 0.1, 0.35, 0.35) will be
  in the top right
...
/*!
  ...
  pos, this method returns a value corresponding to 0.99 times the parent plot's
  selection
...
/*!
  ...
  esHalfBar are
  affected by it, which can be used to control to which side the half bar points
  to.
*/
...
/*! \internal
  ...
  from the position
  the decoration is drawn at (typically a line ending/\ref QCPItemPosition of an
  item).

  This is relevant for clipping. Only omit painting of the decoration when the
  position where the
  decoration is supposed to be drawn is farther away from the clipping rect than
  the returned
  distance.
*/
...
/*!
  ...
  esFlatArrow, even if
  both have the same \ref setLength value, because the spike arrow has an inward
  curved back, which
...
/*! \class QCPGrid
  ...
  uses it to draw the
  grid lines, sub grid lines and zero-line. You can interact with the grid of an
  axis via \ref
...
/*! \fn void QCPAxis::ticksRequest()
  ...
  provide ticks (and
  maybe labels) dynamically, e.g. depending on the current axis range, connect a
  slot to this
...
/*!
  ...
  scaling. In logarithmic axis scaling, major tick marks appear at all powers of
  \a base.
...
/*!
  ...
  the \a lower part
  of the \ref setRange interface will still reference the mathematically smaller
  number than the \a
  upper part.
*/
...
/*!
  ...
  generated tick step or a tick step provided manually via \ref setTickStep, see
  \ref setAutoTickStep).
...
/*!
  ...
  described above in
  milliseconds. Divide its return value by 1000.0 to get a value with the format
  needed for
  ...
  accuracy, use of
  toMSecsSinceEpoch()/1000.0 should be preferred as key coordinate for date/time
  axes.
...
/*!
  ...
  drawn normally. Else,
  the tick labels are drawn rotated by \a degrees clockwise. The specified angle
  is bound to values
...
/*!
  ...
  require tick labels
  to be inside the axis rect, to save space. If \a side is set to \ref lsInside,
  the tick labels
...
/*!
  ...
  is a centered dot.
  If instead a cross should be shown (as is usual in the USA), the third char of
  \a formatCode can
...
/*!
  ...
  powers), the display
  usually is "1 [multiplication sign] 10 [superscript] n", which looks unnatural
  for logarithmic
...
/*!
  ...
  be disabled, else
  the provided tick vector will be overwritten with automatically generated tick
  coordinates upon
...
/*!
  ...
  number of QStrings that will be displayed at the tick positions which you need
  to provide with
...
/*!
  ...
  ticks will reach
  outside the plot. If \a outside is greater than zero, the tick labels and axis
  label will
...
/*!
  ...
  For example, if \a
  factor is 2.0, \a center is 1.0, then the axis range will double its size, and
  the point at
  coordinate 1.0 won't have changed its position in the QCustomPlot widget (i.e.
  coordinates
...
/*!
  ...
  won't have the desired effect, since the widget dimensions aren't defined yet,
  and a resizeEvent
  will follow.
*/
...
void QCPAxis::rescale(bool onlyVisiblePlottables) {
  ...
  if (haveRange) {
    if (!QCPRange::validRange(newRange)) // likely due to range being zero
                                         // (plottable has only constant data in
                                         // this axis dimension), shift current
                                         // range to at least center the
                                         // plottable
    {
      double center = (newRange.lower + newRange.upper) *
                      0.5; // upper and lower should be equal anyway, but just
                           // to make sure, incase validRange returned false for
                           // other reason
      if (mScaleType == stLinear) {
...
double QCPAxis::coordToPixel(double value) const {
  if (orientation() == Qt::Horizontal) {
    ...
    {
      if (value >= 0 && mRange.upper < 0) // invalid value for logarithmic
                                          // scale, just draw it outside visible
                                          // range
        return !mRangeReversed ? mAxisRect->right() + 200
                               : mAxisRect->left() - 200;
      else if (value <= 0 && mRange.upper > 0) // invalid value for logarithmic
                                               // scale, just draw it outside
                                               // visible range
...
/*! \internal
  ...
  setupTickVectors to
  generate reasonable tick positions (and subtick count). The algorithm tries to
  create
...
void QCPAxis::generateAutoTicks() {
  ...
  } else // mScaleType == stLogarithmic
  {
    ...
    if (mRange.lower > 0 && mRange.upper > 0) // positive range
    {
      ...
      while (currentMag < mRange.upper && currentMag > 0) // currentMag might be
                                                          // zero for ranges
                                                          // ~1e-300, just
                                                          // cancel in that case
      {
...
/*! \internal
  ...
  is appropriate. For
  Example taking 4 sub ticks for a \a tickStep of 1 makes more sense than taking
  5 sub ticks,
...
void QCPAxis::draw(QCPPainter *painter) {
  ...
  if (mTicks) {
    ...
    if (mSubTickCount > 0) {
      ...
      for (int i = 0; i < subTickCount; ++i) // no need to check bounds because
                                             // subticks are always only created
...
void QCPAxisPainterPrivate::draw(QCPPainter *painter) {
  ...
  if (reversedEndings)
    baseLine = QLineF(baseLine.p2(), baseLine.p1()); // won't make a difference
                                                     // for line itself, but for
                                                     // line endings later
...
/*! \internal
  ...
  have changed such
  that the cached labels must be refreshed (\ref clearCache). It is used in \ref
  draw. If the
...
/*! \internal
  ...
  drawTickLabel calls during the process of drawing all tick labels of one axis.
  In every call, \a
...
void QCPAxisPainterPrivate::placeTickLabel(QCPPainter *painter, double position,
          ...
          QCPPainter::pmNoCaching)) // label caching enabled
  {
    ...
    mLabelCache.insert(text, cachedLabel); // return label to cache or insert
                                           // for the first time if newly
                                           // created
...
/*! \internal
  ...
  can then be further
  processed by \ref getTickLabelDrawOffset and \ref drawTickLabel. It splits the
  text into base and
...
QCPAxisPainterPrivate::getTickLabelData(const QFont &font,
  ...
  if (result.baseFont.pointSizeF() > 0) // might return -1 if specified with
                                        // setPixelSize, in that case we can't
                                        // do correction in next line
    result.baseFont.setPointSizeF(result.baseFont.pointSizeF() +
                                  0.05); // QFontMetrics.boundingRect has a bug
                                         // for exact point sizes that make the
                                         // results oscillate due to internal
                                         // rounding
  if (useBeautifulPowers) {
    ...
    while (result.expPart.length() > 2 &&
           result.expPart.at(1) == QLatin1Char('0')) // length > 2 so we leave
                                                     // one zero when
  ...
  result.totalBounds.moveTopLeft(QPoint(0, 0)); // want bounding box aligned top
                                                // left at origin, independent
                                                // of how it was created, to
...
QPointF QCPAxisPainterPrivate::getTickLabelDrawOffset(
  ...
  /*
    ...
    appearance): short
    explanation for bottom axis: The anchor, i.e. the point in the label that is
    placed
    horizontally under the corresponding tick is always on the label side that
    is closer to the
    axis (e.g. the left side of the text when we're rotating clockwise). On that
    side, the height
    ...
    height). At the same
    time, a 45 degree rotated text will "point toward" its tick, as is typical
    for rotated tick
    labels.
  */
...
/*! \internal
  ...
  of the text label
  to be drawn, depending on number format etc. Since only the largest tick label
  is wanted for the
...
/*!
  ...
  \ref
  QCustomPlot::setInteractions contains iSelectPlottables. You only need to call
  this function when
...
/*!
  ...
  the plottable
  needs a more specialized representation in the legend, this function will take
  this into account
...
/*! \class QCPItemPosition
  ...
  as absolute pixel
  coordinates, as plot coordinates of certain axes, etc. For more advanced plots
  it is also
  ...
  point of the line
  always be centered under the text label, no matter where the text is moved to.
  For more advanced
  plots, it is possible to assign different parent anchors per X/Y coordinate of
  the position, see
...
/*! \fn QCPItemAnchor *QCPItemPosition::parentAnchor() const
  ...
  setParentAnchorY),
  this method returns the parent anchor of the Y coordinate. In that case rather
  use \a
...
/*!
  ...
  directly, even if
  you want to make a new item subclass. Use \ref QCPAbstractItem::createPosition
  instead, as
  explained in the subclassing section of the QCPAbstractItem documentation.
*/
...
/*!
  ...
  ranges. This
  corresponds to all other types, i.e. \ref ptAbsolute, \ref ptViewportRatio and
  \ref
  ptAxisRectRatio. They differ only in the way the absolute position is
  described, see the
  documentation of \ref PositionType for details. For \ref ptAxisRectRatio, note
...
void QCPItemPosition::setTypeX(QCPItemPosition::PositionType type) {
  if (mPositionTypeX != type) {
    ...
    // axes or axis rect
    // were deleted), don't try to recover the pixelPoint() because it would
    // output a qDebug warning.
...
bool QCPItemPosition::setParentAnchorX(QCPItemAnchor *parentAnchor,
  ...
  while (currentParent) {
    if (QCPItemPosition *currentParentPos =
      ...
      // items aren't the
      // same, to prevent a position being child of an anchor which itself
...
QPointF QCPItemPosition::pixelPoint() const {
  ...
  case ptAxisRectRatio: {
    if (mAxisRect) {
      ...
      qDebug() << Q_FUNC_INFO << "Item position type x is ptAxisRectRatio, but "
                                 "no axis rect was defined";
    break;
  }
  case ptPlotCoords: {
  ...
  case ptAxisRectRatio: {
    if (mAxisRect) {
      ...
      qDebug() << Q_FUNC_INFO << "Item position type y is ptAxisRectRatio, but "
                                 "no axis rect was defined";
    break;
  }
  case ptPlotCoords: {
...
/*!
  ...
  setType) this
  QCPItemPosition is or what parent-child situation it is in, as coordinates are
  transformed
...
/*! \class QCPAbstractItem
  ...
  plottables
  (QCPAbstractPlottable) nor axes (QCPAxis). While plottables are always tied to
  two axes and thus
  plot coordinates, items can also be placed in absolute coordinates independent
  of any axes. Each
  ...
  assume the new item shall
  have only one point as its position (as opposed to two like a rect or multiple
  like a polygon). You then add
  ...
  own enum for this).
  Since anchors can be placed anywhere, relative to the item's position(s), your
  item needs to
...
/*! \internal
  ...
  times, once for each
  rect, in your \ref selectTest reimplementation. Finally, return the minimum of
  all four returned
  values.
...
/*! \internal
  ...
  to it. The specified
  \a name must be a unique string that is usually identical to the variable name
  of the position
...
/*! \fn void QCustomPlot::mouseMove(QMouseEvent *event)
  ...
  a meaning for the range drag axes that were set at that moment. If you want to
  change the drag
...
/*! \fn void QCustomPlot::mouseRelease(QMouseEvent *event)
  ...
  selection. So a
  slot connected to this signal can still influence the behaviour e.g. with \ref
  setInteractions or
...
/*! \fn void QCustomPlot::selectionChangedByUser()
  ...
  signals, for example
  QCPAxis::selectionChanged or QCPAbstractPlottable::selectionChanged. Note that
  those signals are
...
/*! \var QCPAxis *QCustomPlot::xAxis
  ...
  thelayoutsystem the
  layout system\endlink to add multiple axis rects or multiple axes to one side,
  use the \ref
...
/*!
  ...
  QCP::iRangeZoom. When the
  respective interaction is enabled, the user may drag axes ranges and zoom with
  the mouse wheel.
  ...
  QCPAbstractPlottable::selected(). To retrieve a list of all currently selected
  plottables, call
...
/*!
  ...
  background pixmap
  was set with \ref setBackground(const QPixmap &pm), this brush will be used to
  fill the viewport
...
QCPAbstractItem *QCustomPlot::itemAt(const QPointF &pos,
  ...
  foreach (QCPAbstractItem *item, mItems) {
    if (onlySelectable && !item->selectable()) // we could have also passed
                                               // onlySelectable to the
                                               // selectTest function, but
                                               // checking here is faster,
                                               // because we have access to
                                               // QCPAbstractItem::selectable
      continue;
    if (!item->clipToAxisRect() ||
        item->clipRect().contains(pos.toPoint())) // only consider clicks inside
                                                  // axis cliprect of the item
                                                  // if actually clipped to it
    {
...
void QCustomPlot::replot(QCustomPlot::RefreshPriority refreshPriority) {
  ...
  if (painter.isActive()) {
    ...
    qDebug() << Q_FUNC_INFO << "Couldn't activate painter on buffer. This "
                               "usually happens because QCustomPlot has width "
                               "or height zero.";
...
/*!
  ...
  This means, the
  output will look like the normal on-screen output of a QCustomPlot widget with
  the corresponding
  ...
  (default), this
  function uses the current width and height of the QCustomPlot widget. However,
  in Qt, these
...
/*!
  ...
  dimensions \a width
  and \a height in pixels. If either \a width or \a height is zero, the exported
  image will have
...
void QCustomPlot::mousePressEvent(QMouseEvent *event) {
  ...
  mMousePressPos = event->pos(); // need this to determine in releaseEvent
                                 // whether it was a click (no position change
...
/*! \internal
  ...
  the \ref
  mousePressEvent, it is considered a click which causes the selection mechanism
  (if activated via
...
void QCustomPlot::mouseReleaseEvent(QMouseEvent *event) {
      ...
      5) // determine whether it was a click operation
  {
    ...
    QVariant details;
    QCPLayerable *clickedLayerable =
        layerableAt(event->pos(), false, &details); // for these signals,
                                                    // selectability is ignored,
                                                    // that's why we call this
                                                    // again with onlySelectable
                                                    // set to false
    if (QCPAbstractPlottable *ap =
            qobject_cast<QCPAbstractPlottable *>(clickedLayerable))
...
/*!
  ...
  format. The plot is
  sized to \a width and \a height in pixels and scaled with \a scale. (width 100
  and scale 2.0 lead
...
QPixmap QCustomPlot::toPixmap(int width, int height, double scale) {
  ...
  if (painter.isActive()) {
    ...
    if (!qFuzzyCompare(scale, 1.0)) {
      if (scale > 1.0) // for scale < 1 we always want cosmetic pens where
                       // possible, because else lines might disappear for very
                       // small scales
        painter.setMode(QCPPainter::pmNonCosmetic);
      painter.scale(scale, scale);
    }
    if (mBackgroundBrush.style() != Qt::SolidPattern &&
        mBackgroundBrush.style() != Qt::NoBrush) // solid fills were done a few
                                                 // lines above with
                                                 // QPixmap::fill
      painter.fillRect(mViewport, mBackgroundBrush);
    draw(&painter);
...
void QCustomPlot::toPainter(QCPPainter *painter, int width, int height) {
  ...
  if (painter->isActive()) {
    ...
    setViewport(QRect(0, 0, newWidth, newHeight));
    painter->setMode(QCPPainter::pmNoCaching);
    if (mBackgroundBrush.style() != Qt::NoBrush) // unlike in toPixmap, we can't
                                                 // do QPixmap::fill for
                                                 // Qt::SolidPattern brush
                                                 // style, so we also draw solid
...
/*! \class QCPColorGradient
  ...
  color. For example,
  QCPColorMap and QCPColorScale have \ref QCPColorMap::setGradient "setGradient"
  methods which
  take an instance of this class. Colors are set with \ref setColorStopAt(double
  ...
  that lie outside
  the data range specified on the plottable instance can be controlled with \ref
  setPeriodic.
*/
...
/*!
  ...
  the keys, the
  colors are the values of the passed QMap \a colorStops. In between these color
  stops, the color
...
void QCPColorGradient::updateColorBuffer() {
  ...
  if (mColorStops.size() > 1) {
    ...
    for (int i = 0; i < mLevelCount; ++i) {
      ...
      } else if (it == mColorStops.constBegin()) // position is on or before
                                                 // first stop, use color of
                                                 // first stop
      {
...
QCPAxis *QCPAxisRect::addAxis(QCPAxis::AxisType type, QCPAxis *axis) {
  ...
  } else // user provided existing axis instance, do some sanity checks
  {
    if (newAxis->axisType() != type) {
      qDebug() << Q_FUNC_INFO << "passed axis has different axis type than "
...
/*!
  ...
  <tt>or</tt>-combination of QCPAxis::AxisType, so axes can be added to multiple
  sides at once.
...
/*!
  ...
  and left axes are connected to the \ref QCPAxis::setRange slots of the top and
  right axes.
*/
...
/*!
  ...
  QCPAxis *vertical). By default, the horizontal axis is the bottom axis (xAxis)
  and the vertical
...
void QCPAxisRect::updateAxesOffset(QCPAxis::AxisType type) {
  ...
  for (int i = 1; i < axesList.size(); ++i) {
    ...
    if (axesList.at(i)->visible()) // only add inner tick length to offset if
                                   // this axis is visible and it's not the
                                   // first visible one (might happen if true
                                   // first axis is invisible)
    {
      if (!isFirstVisible)
...
void QCPAxisRect::mousePressEvent(QMouseEvent *event) {
  mDragStart = event->pos(); // need this even when not LeftButton is pressed,
                             // to determine in releaseEvent whether it was a
                             // full click (no position change between press and
                             // release)
...
/*! \internal
  ...
  the scaling operation is the current cursor position inside the axis rect. The
  scaling factor is
...
void QCPPlottableLegendItem::draw(QCPPainter *painter) {
  ...
  int textHeight = qMax(textRect.height(),
                        iconSize.height()); // if text has smaller height than
                                            // icon, center text vertically in
                                            // icon height, else align tops
...
QCPLegend::~QCPLegend() {
  ...
  if (qobject_cast<QCustomPlot *>(mParentPlot)) // make sure this isn't called
                                                ...
                                                // destructed (happens when the
                                                // legend is not in any layout
                                                // and thus QObject-child of
                                                // QCustomPlot)
...
/*!
  ...
  to it.
  Legend items that draw an icon (e.g. a visual representation of the graph) and
  text (e.g. the
...
/*! \fn QCPAxis *QCPColorScale::axis() const
  ...
  (\ref
  QCPAxis::setLabel). As they each are connected, it does not matter whether you
  use the method on
...
void QCPColorScale::setType(QCPAxis::AxisType type) {
  ...
  if (mType != type) {
    ...
    mColorAxis.data()->setRange(rangeTransfer); // transfer range of old axis to
                                                // new one (necessary if axis
...
/*!
  ...
  color maps. It is
  also equivalent to directly accessing the \ref axis and setting its range with
  \ref
  QCPAxis::setRange.
...
/*!
  ...
  also equivalent to directly accessing the \ref axis and setting its scale type
  with \ref
...
void QCPColorScale::rescaleDataRange(bool onlyVisibleMaps) {
  ...
  if (haveRange) {
    if (!QCPRange::validRange(newRange)) // likely due to range being zero
                                         // (plottable has only constant data in
                                         // this dimension), shift current range
...
/*! \class QCPGraph
  ...
  the key axis of the graph, and fills between two graphs, called channel fills.
  To enable a fill,
...
/*!
  ...
  false, the graph
  takes ownership of the passed data and replaces the internal data pointer with
  it. This is
...
/*!
  ...
  to actually pass
  error data via the specific setData functions along with the data points (e.g.
  \ref
  ...
  if (mLineStyle != lsNone) {
    ...
    painter->drawLine(
        QLineF(rect.left(), rect.top() + rect.height() / 2.0, rect.right() + 5,
               rect.top() + rect.height() / 2.0)); // +5 on x2 else last segment
                                                   // is missing from
                                                   // dashed/dotted pens
  }
  // draw scatter symbol:
...
/*! \internal
  ...
  scatter symbols accordingly. If no scatters need to be drawn, i.e. the scatter
  style's shape is
...
void QCPGraph::getLinePlotData(QVector<QPointF> *linePixelData,
  ...
  linePixelData->reserve(lineData.size() + 2); // added 2 to reserve memory for
                                               // lower/upper fill base points
...
void QCPGraph::getStepLeftPlotData(QVector<QPointF> *linePixelData,
  ...
  QVector<QCPData> lineData;
  getPreparedData(&lineData, scatterData);
  linePixelData->reserve(lineData.size() * 2 + 2); // added 2 to reserve memory
      ...
      while (i < lineDataSize) {
            ...
            qIsNaN(lineData->at(i).x())) // NaNs create a gap in the line
        {
          painter->drawPolyline(lineData->constData() + segmentStart,
                                i - segmentStart); // i, because we don't want
                                                   // to include the current NaN
                                                   // point
          segmentStart = i + 1;
        }
        ++i;
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData,
  ...
  QCPDataMap::const_iterator lower, upper; // note that upper is the actual
                                           // upper point, and not 1 step after
                                           // the upper point
  ...
  if (mAdaptiveSampling && dataCount >= maxCount) // use adaptive sampling only
                                                  // if there are at least two
                                                  // points per pixel on average
  {
    if (lineData) {
      ...
      double keyEpsilon = qAbs(
          ...
          keyAxis->pixelToCoord(keyAxis->coordToPixel(currentIntervalStartKey) +
                                1.0 * reversedFactor)); // interval of one pixel
                                                        // on screen when mapped
                                                        // to plot key
                                                        // coordinates
        ...
        } else // new pixel interval started
        {
                                      ...
                                      // consolidate them to a cluster
          {
            if (lastIntervalEndKey <
                currentIntervalStartKey - keyEpsilon) // last point is further
                                                      // away, so first point of
                                                      // this cluster must be at
                                                      // a real data point
              lineData->append(
            ...
            if (it.key() > currentIntervalStartKey +
                               keyEpsilon * 2) // new pixel started further away
                                               // from previous cluster, so make
                                               // sure the last point of the
                                               // cluster is at a real data
                                               // point
              lineData->append(
      ...
      {
        if (lastIntervalEndKey <
            currentIntervalStartKey - keyEpsilon) // last point wasn't a
                                                  // cluster, so first point of
                                                  // this cluster must be at a
                                                  // real data point
    ...
    if (scatterData) {
      ...
      int reversedRound =
          keyAxis->rangeReversed() ? 1 : 0; // is used to switch between floor
                                            // (normal) and ceil (reversed)
                                            // rounding of
                                            // currentIntervalStartKey
      double currentIntervalStartKey = keyAxis->pixelToCoord(
          (int)(keyAxis->coordToPixel(lower.key()) + reversedRound));
          ...
          2) // last pixel had multiple data points, consolidate them
      {
        ...
        int dataModulo =
            qMax(1, qRound(intervalDataCount /
                           (valuePixelSpan / 4.0))); // approximately every 4
...
void QCPGraph::drawError(QCPPainter *painter, double x, double y,
  ...
  double skipSymbolMargin = mScatterStyle.size(); // pixels left blank per side,
                                                  // when mErrorBarSkipSymbol is
                                                  // true
...
/*!  \internal
  ...
  only needs to be done until \a maxCount is reached, which should be set to the
  number of data
...
/*! \internal
  ...
  that connects the
  data points. If the graph needs to be filled, two additional points need to be
  added at the
...
/*! \internal
  ...
  on whether the key
  axis is horizontal or vertical, \a lowerKey will end up as the x or y value of
  the returned
  point, respectively.
...
/*! \internal
  ...
  called by \ref addFillBasePoints to conveniently assign the point which closes
  the fill
  polygon on the upper side of the zero-value-line parallel to the key axis. The
  logarithmic axis
  scale case is a bit special, since the zero-value-line in pixel coordinates is
  in positive or
  negative infinity. So this case is handled separately by just closing the fill
  polygon on the
  ...
  on whether the key
  axis is horizontal or vertical, \a upperKey will end up as the x or y value of
  the returned
  point, respectively.
...
QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const {
  ...
  thisData.reserve(lineData->size() + otherData.size()); // because we will join
                                                         // both vectors at end
                                                         // of this function
  for (int i = 0; i < lineData->size(); ++i) // don't use the vector<<(vector),
                                             // it squeezes internally, which
                                             // ruins the performance tuning
                                             // with reserve()
  ...
  } else // mKeyAxis->orientation() == Qt::Vertical
  {
    ...
    // inverted compared to x,
    // because in pixel coordinates, y increases from top to bottom, not bottom
    // to top like data coordinate.
    // if an axis range is reversed, the data point keys will be descending.
    // Reverse them, since following algorithm assumes ascending keys:
...
/*! \internal
  ...
  is \ref QCPScatterStyle::ssNone (i.e. there is no visual representation of the
  graph), returns -1.0.
*/
...
QCPRange QCPGraph::getKeyRange(bool &foundRange,
  ...
  // whether error bars
  // should also be taken into consideration for range calculation. We set this
  // to true here.
             ...
             sdPositive) // range may only be in the positive sign domain
  {
    ...
    while (it != mData->constEnd()) {
      if (!qIsNaN(it.value().value)) {
        ...
        if (includeErrors) // in case point is in valid sign domain but errobars
                           // stretch beyond it, we still want to get that
                           // point.
        {
          if ((current < range.lower || !haveLower) && current > 0) {
            range.lower = current;
...
/*!
  ...
  \a fromt is
  greater or equal to \a tot, the function does nothing. To remove a single data
  point with known
...
void QCPCurve::getCurveData(QVector<QPointF> *lineData) const {
  ...
  while (it != mData->constEnd()) {
    currentRegion = getRegion(it.value().key, it.value().value, rectLeft,
                              rectTop, rectRight, rectBottom);
    if (currentRegion != prevRegion) // changed region, possibly need to add
                                     // some optimized edge points or original
                                     // points if entering R
    {
      if (currentRegion !=
      ...
      {
        if (it == mData->constBegin()) // it is first point in curve and prevIt
                                       // is last one. So save optimized point
                                       // for adding it to the lineData in the
                                       // end
...
/*! \internal
  ...
  outwards. In the
  curve optimization algorithm, region 5 is considered to be the visible portion
  of the plot.
*/
...
/*! \internal
  ...
  exit points of the
  segment into/out of region 5, as \ref getTraverse provides. It may happen that
  a single segment, in
...
/*! \class QCPBarsGroup
  ...
  it may be desirable
  to have them appearing next to each other at each key. This is what adding the
  respective QCPBars
...
/*! \internal
  ...
  should have at the
  given key coordinate \a keyCoord. The offset is relative to the pixel position
  of the key
...
double QCPBarsGroup::keyPixelOffset(const QCPBars *bars, double keyCoord) {
  ...
  if (index >= 0) {
               ...
               (baseBars.size() - 1) / 2.0) // bar is to the left of center
    {
      ...
      for (int i = startIndex; i > index; --i) // add widths and spacings of
                                               // bars in between center and our
                                               // bars
      {
...
/*! \internal
  ...
  \a bars is only
  needed to get acces to the key axis transformation and axis rect for the modes
  \ref
...
/*! \overload
  ...
  consider using \ref removeData(double fromKey, double toKey) with a small
  fuzziness interval
  around the suspected position, depeding on the precision with which the key is
  known.
...
/*! \internal
  ...
  baseValue upwards,
  negative are stacked below baseValue downwards). This can be indicated with \a
  positive. So if the
...
/*! \class QCPStatisticalBox
  ...
  Qt::FlatCap. Else, the
  backbone line might exceed the whisker bars by a few pixels due to the pen cap
  being not
  perfectly flat.
...
/*!
  ...
  of the lower
  whisker, typically the minimum measurement of the sample that's not considered
  an outlier.
...
/*!
  ...
  of the upper
  whisker, typically the maximum measurement of the sample that's not considered
  an outlier.
...
/*!
  ...
  the value range, This defines the rectangular area covered by the color map in
  plot coordinates.
...
/*!
  ...
  you shouldn't use the \ref QCPColorMapData::setData method as it uses a linear
  transformation to
...
/*!
  ...
  axis, both not
  range-reversed), the cell with indices (0, 0) is in the bottom left corner and
  the cell with
...
/*!
  ...
  instance. The resulting cell indices are returned via the output parameters \a
  keyIndex and \a
  valueIndex.
...
/*!
  ...
  colored uniformly with the respective gradient boundary color, or the gradient
  will repeat,
...
/*!
  ...
  QCPColorMapData
  instance (\ref data). As data is updated via its \ref QCPColorMapData::setCell
  or \ref
  QCPColorMapData::setData, the buffered minimum and maximum values are updated,
  too. For
...
/*! \internal
  ...
  without smooth transform enabled. Accordingly, oversampling isn't performed if
  \ref
...
void QCPColorMap::updateMapImage() {
  ...
  int keyOversamplingFactor =
      mInterpolate
          ? 1
          : (int)(1.0 + 100.0 / (double)keySize); // make mMapImage have at
  ...
  QImage *localMapImage = &mMapImage; // this is the image on which the
                                      // colorization operates. Either the final
                                      // mMapImage, or if we need oversampling,
  ...
  if (keyOversamplingFactor > 1 || valueOversamplingFactor > 1) {
    ...
    mUndersampledMapImage = QImage(); // don't need oversampling mechanism
                                      ...
                                      // mUndersampledMapImage still has nonzero
                                      // size, free it
  ...
  if (keyAxis->orientation() == Qt::Horizontal) {
    ...
    for (int line = 0; line < lineCount; ++line) {
      QRgb *pixels = reinterpret_cast<QRgb *>(localMapImage->scanLine(
          lineCount - 1 - line)); // invert scanline index because QImage counts
  ...
  if (keyAxis->orientation() == Qt::Horizontal) {
    for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it) {
      ...
      double keyWidthPixels =
          keyPixel -
          keyAxis->coordToPixel(it.value().key - mWidth * 0.5); // sign of this
                                                                // makes sure
                                                                // open/close
                                                                // are on
                                                                // correct sides
...
/*! \internal
  ...
  the text is finally
  drawn at that point, the lower left corner of the resulting text rect is at \a
  pos.
*/
...
/*! \class QCPItemTracer
  ...
  the key stays controllable via \ref setGraphKey, but the value will follow the
  graph data. If a
  QCPGraph is connected, note that setting the coordinates of the tracer item
  directly via \a
  position will have no effect because they will be overriden in the next redraw
  (this is when the
  ...
  to make the tracer
  have no own visual appearance (set the style to \ref tsNone), and just connect
  other item
