...
Motolink::Motolink(QObject *parent) : QObject(parent) {
  mUsb = new QUsbDevice;
  mThread = new QThread;
...
QString Motolink::getVersion(quint8 idx) {
  ...
  if (this->sendCmd(&send, &recv, sizeof(version_t) * 2, CMD_GET_VERSION)) {
    ...
    return QString("%1.%2.%3")
        .arg(mVersion[idx].major)
        .arg(mVersion[idx].minor)
        .arg(mVersion[idx].patch);
  }
  return tr("Error");
}
...
bool Motolink::readTables(void) {
  ...
  if (this->sendCmd(&send, &recv, size, CMD_GET_TABLES)) {
    ...
    memcpy((void *)mKnockTable, (void *)(recv.constData() + sizeof(mAFRTable)),
           sizeof(mKnockTable));
...
bool Motolink::clearTables() { return this->sendSimpleCmd(CMD_CLEAR_TABLES); }
...
bool Motolink::sendWake() { return this->sendSimpleCmd(CMD_WAKE); }
...
bool Motolink::resetDevice() { return this->sendSimpleCmd(CMD_RESET); }
...
bool Motolink::sendCmd(QByteArray *send, QByteArray *recv, uint len,
                       quint8 cmd) {
  _WAIT_USB_
  _LOCK_
  bool result;
...
bool Motolink::verifyFirmware(QByteArray *data) {
  ...
  for (int i = 0; i < data->size(); i += buf_size) {
    ...
    _UNLOCK_
    if (read_size <
        data_local.size()) { // Read same amount of data as from file.
    ...
    if (data_remote != data_local) {
      ...
      emit transferProgress(100);
      emit signalStatus(tr("Verification failed at 0x") +
    ...
    // emit sendStatus(tr("Verified ")+QString::number(i/1024)+tr(" kilobytes
    // out of ")+QString::number(data->size()/1024));
  }
