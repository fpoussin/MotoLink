...
qint32 QUsb::open()
{
    ...
    else if (!QueryDeviceEndpoints(m_usbHandle, &m_pipeId)) return -1;

//    m_pipeId.PipeInId = 0x81;
//    m_pipeId.PipeOutId = 0x01;
...
//    ulong stall = 1; // ms
//    if (!WinUsb_SetPipePolicy(m_usbHandle, m_pipeId.PipeInId, AUTO_CLEAR_STALL, sizeof(stall), &stall)) {
//        qCritical("Error WinUsb_SetPipePolicy: %d.\n", GetLastError()); return -1; }
    bool enable = true; // ms
   // if (!WinUsb_SetPipePolicy(m_usbHandle, m_pipeId.PipeInId, SHORT_PACKET_TERMINATE, sizeof(enable), &enable)) {
   //     qCritical("Error WinUsb_SetPipePolicy: %d.\n", GetLastError()); return -1; }

    if (!WinUsb_SetPipePolicy(m_usbHandle, m_pipeId.PipeInId, IGNORE_SHORT_PACKETS, sizeof(enable), &enable)) {
        qCritical("Error WinUsb_SetPipePolicy: %d.\n", GetLastError()); return -1; }
...
qint32 QUsb::write(QByteArray *buf, quint32 bytes)
{
    ...
    qDebug() << "Sent" << cbSent << "/" << buf->size() << "bytes:" << cmd;
    if (!bResult) {
        PrintUsbError("WinUsb_WritePipe");
        //WinUsb_FlushPipe(m_usbHandle, m_pipeId.PipeOutId);
        //WinUsb_ResetPipe(m_usbHandle, m_pipeId.PipeOutId);
        return -1;
    }
    return cbSent;
}
