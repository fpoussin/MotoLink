...
def sendSrc(ver):
	print check_output(["dput ppa:mobyfab/motolink "+ver+"_source.changes"], shell=True)
...
if __name__ == "__main__":
	...
	ver = check_output(["grep \"VERSION =\" ../MotoLink.pro | awk '{ print $3 }' "], shell=True).replace('\n','')
	if not ver:
	...
	folder_name = 'motolink-'+str(ver)+'~'+args.release
	...
	if args.source:
		makeSrc(folder_name)
		if args.ppa:
			sendSrc('motolink_'+str(ver)+'~'+args.release)
	if args.bin:
		makeBin(folder_name)
