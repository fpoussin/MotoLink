...
public:
  ...
  quint8 getBtlFlags(void) {
    _LOCK_ quint8 tmp = mBtl->getFlags();
    _UNLOCK_ return tmp;
  }
  bool boot() {
    _LOCK_ bool tmp = mBtl->boot();
    _UNLOCK_ return tmp;
  }
  ...
  inline bool getFunctionAFR_Disabled(void) {
    return mSettings.afrInput == AFR_INPUT_DISABLED;
  }
  inline bool getFunctionAFR_Analog(void) {
    return mSettings.afrInput == AFR_INPUT_AN;
  }
  inline bool getFunctionAFR_MTS(void) {
    return mSettings.afrInput == AFR_INPUT_MTS;
  }
  inline bool getFunctionAFR_OBD(void) {
    return mSettings.afrInput == AFR_INPUT_OBD;
  }
  inline bool getFunctionRecording(void) {
    return mSettings.functions & FUNC_RECORD;
  }
  inline bool getFunctionInput_Direct(void) {
    return mSettings.sensorsInput == SENSORS_INPUT_DIRECT;
  }
  inline bool getFunctionInput_OBD(void) {
    return mSettings.sensorsInput == SENSORS_INPUT_OBD_CAN;
  }
  inline bool getFunctionInput_Yamaha(void) {
    return mSettings.sensorsInput == SENSORS_INPUT_YAMAHA_CAN;
  }
  ...
  inline void setFunctionRecord(bool on) {
    if (on)
      mSettings.functions |= FUNC_RECORD;
    else
      mSettings.functions &= ~FUNC_RECORD;
  }

  inline void setFunctionInput_Direct(void) { mSettings.sensorsInput = 0; }
  inline void setFunctionInput_OBD(void) {
    mSettings.sensorsInput = SENSORS_INPUT_OBD_CAN;
  }
  inline void setFunctionInput_Yamaha(void) {
    mSettings.sensorsInput = SENSORS_INPUT_YAMAHA_CAN;
  }
