...
CCM_FUNC uint8_t readCommand(BaseChannel *chn, uint8_t flags) {
  ...
  if (chnReadTimeout(chn, (uint8_t *)&header, sizeof(cmd_header_t),
                     TIME_MS2I(50)) < sizeof(cmd_header_t)) {
  ...
  data_read = (uint8_t)chnReadTimeout(
      chn, data_buf, header.len - sizeof(header), TIME_MS2I(25));
...
CCM_FUNC uint8_t writeHandler(BaseChannel *chn, uint8_t *buf, uint8_t len) {
  ...
  if (res != 0)
    replbuf = MASK_CMD_ERR | CMD_WRITE;
...
CCM_FUNC uint8_t readHandler(BaseChannel *chn, uint8_t *buf) {
  ...
  chnWriteTimeout(chn, (uint8_t *)(address + USER_APP_ADDR), buf_len,
                  TIME_MS2I(50));
  return 0;
}
...
CCM_FUNC uint8_t sendVersion(BaseChannel *chn) {
  ...
  chnWriteTimeout(chn, (uint8_t *)versions, sizeof(version_t) * 2,
                  TIME_MS2I(50));
  return 0;
}
