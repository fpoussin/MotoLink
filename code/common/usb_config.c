...
static const USBDescriptor usb_device_descriptor = {
    sizeof usb_device_descriptor_data, usb_device_descriptor_data};

#define BASE_IF_DESC_SET_SIZE                                                  \
  ((USB_DESC_INTERFACE_SIZE * 2) + (USB_DESC_ENDPOINT_SIZE * 3))
...
#define BASE_IF_DESC_SET(comIfNum, datIfNum, comInEp, datOutEp, datInEp)       \
  ...
  USB_DESC_INTERFACE(                                                          \
      ...
      0x04), /* iInterface.           */ /* Endpoint, Interrupt IN.*/          \
      USB_DESC_ENDPOINT(comInEp, USB_EP_MODE_TYPE_INTR, /* bmAttributes. */                                                                            \
      ...
      USB_DESC_INTERFACE(                                                      \
          ...
          0x00,                                 /* bInterfaceSubClass       */ \
          0x00,                                 /* bInterfaceProtocol       */ \
          0x04), /* iInterface.              */ /* Endpoint, Bulk OUT.*/       \
      USB_DESC_ENDPOINT(                                                       \
...
#define IAD_BASE_IF_DESC_SET(comIfNum, datIfNum, comInEp, datOutEp, datInEp)   \
  ...
  USB_DESC_INTERFACE_ASSOCIATION(comIfNum, /* bFirstInterface.         */      \
                                 2,        /* bInterfaceCount.         */      \
                                 0xFF,     /* bFunctionClass.          */      \
...
#define CDC_IF_DESC_SET(comIfNum, datIfNum, comInEp, datOutEp, datInEp)        \
  ...
  USB_DESC_INTERFACE(                                                          \
      ...
      USB_DESC_BYTE(                                                           \
          datIfNum), /* bDataInterface.          */ /* Abstract Control        \
                                                       Management Functional   \
                                                       Descriptor.*/           \
      USB_DESC_BYTE(4),                /* bFunctionLength.         */          \
      USB_DESC_BYTE(CDC_CS_INTERFACE), /* bDescriptorType.         */          \
      USB_DESC_BYTE(CDC_ABSTRACT_CONTROL_MANAGEMENT),                          \
      USB_DESC_BYTE(0x02), /* bmCapabilities.          */ /* Union Functional  \
                                                             Descriptor.*/     \
      USB_DESC_BYTE(5),                /* bFunctionLength.         */          \
      USB_DESC_BYTE(CDC_CS_INTERFACE), /* bDescriptorType.         */          \
      ...
      USB_DESC_BYTE(                                                           \
          datIfNum), /* bSlaveInterface.         */     /* Endpoint, Interrupt \
                                                           IN.*/               \
      USB_DESC_ENDPOINT(comInEp, USB_EP_MODE_TYPE_INTR, /* bmAttributes. */                                                                            \
                        USB_INTERRUPT_REQUEST_SIZE,     /* wMaxPacketSize.     */                                                                        \
                        0x01), /* bInterval.               */                  \
      ...
      USB_DESC_INTERFACE(                                                      \
          ...
          0x00,                                 /* bInterfaceProtocol (CDC     \
                                                   section 4.7).            */ \
          0x03), /* iInterface.              */ /* Endpoint, Bulk OUT.*/       \
      USB_DESC_ENDPOINT(                                                       \
          datOutEp,                             /* bEndpointAddress.        */ \
          USB_EP_MODE_TYPE_BULK,                /* bmAttributes.            */ \
          USB_DATA_SIZE,                        /* wMaxPacketSize.          */ \
...
static const uint8_t usb_configuration_descriptor_data[] = {
    ...
    USB_DESC_CONFIGURATION(
        USB_DESC_CONFIGURATION_SIZE +
            (IAD_CDC_IF_DESC_SET_SIZE +
             IAD_BASE_IF_DESC_SET_SIZE), /* wTotalLength.                */
    ...
    IAD_CDC_IF_DESC_SET(
        USB_CDC_CIF_NUM0, USB_CDC_DIF_NUM0, USB_ENDPOINT_IN(USB_INTERRUPT_EP_A),
        USB_ENDPOINT_OUT(USB_DATA_EP_A), USB_ENDPOINT_IN(USB_DATA_EP_A)),
    IAD_BASE_IF_DESC_SET(USB_BASE_CIF_NUM1, USB_BASE_DIF_NUM1,
...
static const USBDExtPropertiesDesc usb_guid_descriptor_data = {
    .dwLength = sizeof(USBDExtPropertiesDesc) + 1 * sizeof(USBDExtPropertyDesc),
    .bcdVersion = 0x0100,
    .wIndex = 0x0005,
    .bCount = 1,
    .properties = {{.dwPropertySize = sizeof(USBDExtPropertyDesc),
  ...
  if (setup->bmRequestType & (USB_RTYPE_DIR_MASK | USB_RTYPE_TYPE_VENDOR)) {
    if (setup->bRequest == USB_VENDOR_CODE) {
      if (setup->wIndex == USB_FEATURE_COMPAT_ID) {
        usbSetupTransfer(usbp, (uint8_t *)&usb_compat_id_descriptor_data,
                         setup->wLength, NULL);
        return true;
      }

      if (setup->wIndex == USB_FEATURE_PROPERTIES) {
        usbSetupTransfer(usbp, (uint8_t *)&usb_guid_descriptor_data,
                         setup->wLength, NULL);
        return true;
      }
    }
 ...
 */
const USBConfig usbcfg = {usb_event, get_descriptor, requests_hook,
                          sof_handler};

/*
 * Serial over USB driver configuration 1.
 */
const SerialUSBConfig serusbcfg1 = {&USBD1, USB_DATA_EP_A, USB_DATA_EP_A,
                                    USB_INTERRUPT_EP_A};

/*
 * Serial over USB driver configuration 2.
 */
const SerialUSBConfig serusbcfg2 = {&USBD1, USB_DATA_EP_B, USB_DATA_EP_B,
                                    USB_INTERRUPT_EP_B};
