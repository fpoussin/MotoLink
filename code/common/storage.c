...
#define openTablesFS                                                           \
  SPIEepromFileOpen(&tablesFile, &eeTablesCfg,                                 \
                    EepromFindDevice(EEPROM_DEV_25XX))
...
static const CRCConfig crc32_config = {.poly_size = 32,
                                       .poly = 0x04C11DB7,
...
CCM_FUNC static crc_t getCrc(const CRCConfig *config, uint8_t *data,
                             uint16_t len) {
  gCrc = 0;
...
void eeInit(void) { spiStart(&EEPROM_SPID, &EEPROM_SPIDCONFIG); }
...
CCM_FUNC uint32_t eeFindCurrentPageAddr(void) {
  ...
  tablesFS = SPIEepromFileOpen(&tablesFile, &eeTablesCfg,
                               EepromFindDevice(EEPROM_DEV_25XX));
  ...
  for (addr = from; addr < to; addr += EEPROM_TABLES_PAGE_SIZE) {
    ...
    status =
        fileStreamRead(tablesFS, (uint8_t *)&counters[i], sizeof counters[0]);
  ...
  for (i = 0; i < (sizeof counters / sizeof counters[0]); i++) {
    if (counters[i] > max_counter) {
      ...
      addr =
          i *
          EEPROM_TABLES_PAGE_SIZE; // Move address to page with highest counter
    }
  }
...
CCM_FUNC uint8_t eePushPage(int32_t addr, uint8_t *buffer, crc_t *crc,
                            uint32_t len) {
  // Check max length
  ...
  *crc = getCrc(&crc32_config, buffer, len);
  ...
  fileStreamSetPosition(tablesFS,
                        addr + EEPROM_TABLES_PAGE_SIZE - sizeof(crc_t));
  if (fileStreamWrite(tablesFS, (uint8_t *)crc, sizeof(crc_t)) !=
      sizeof(crc_t)) {
    fileStreamClose(tablesFS);
    return 3;
  }
...
CCM_FUNC uint8_t eePullPage(int32_t addr, uint8_t *buffer, crc_t *crc,
                            uint32_t len) {
  // Check max length
  if (len > EEPROM_TABLES_PAGE_SIZE - sizeof(crc_t))
    return 1;
  ...
  fileStreamSetPosition(tablesFS,
                        addr + EEPROM_TABLES_PAGE_SIZE - sizeof(crc_t));
  if (fileStreamRead(tablesFS, (uint8_t *)crc, sizeof(crc_t)) !=
      sizeof(crc_t)) {
    fileStreamClose(tablesFS);
    return 4;
  }
...
CCM_FUNC uint8_t readTablesFromEE(void) {
  ...
  if (eePullPage(eeFindCurrentPageAddr(), (uint8_t *)&tables_buf, &crc1, len) !=
      0)
    return 1;
  ...
  if (crc1 != crc2) {
    // Try previous page
    if (eePullPage(eeFindPrevPageAddr(), (uint8_t *)&tables_buf, &crc1, len) !=
        0)
      return 2;
    crc2 = getCrc(&crc32_config, (uint8_t *)&tables_buf, len);
    if (crc1 != crc2)
...
CCM_FUNC uint8_t writeTablesToEE(void) {
  ...
  res2 =
      eePullPage(eeFindCurrentPageAddr(), (uint8_t *)&tables_buf, &crc2, len);
...
CCM_FUNC uint8_t readSettingsFromEE() {
  ...
  settingsFS = SPIEepromFileOpen(&settingsFile, &eeSettingsCfg,
                                 EepromFindDevice(EEPROM_DEV_25XX));
  ...
  // Fetch CRC from EE
  if (fileStreamRead(settingsFS, (uint8_t *)&crc1, sizeof crc1) !=
      sizeof crc1) {
    fileStreamClose(settingsFS);
    return 2;
  }
...
CCM_FUNC uint8_t readVersionFromEE(uint8_t idx, version_t *dst) {
  ...
  versionsFS = SPIEepromFileOpen(&versionsFile, &eeVersionsCfg,
                                 EepromFindDevice(EEPROM_DEV_25XX));
  ...
  // Fetch CRC from EE
  if (fileStreamRead(versionsFS, (uint8_t *)&crc1, sizeof crc1) !=
      sizeof crc1) {
    fileStreamClose(versionsFS);
    return 2;
  }
