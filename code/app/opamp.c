...
/**
 * @brief  Deinitializes OPAMP peripheral registers to their default reset
 * values.
...
/**
 * @brief  Initializes the OPAMP peripheral according to the specified
 ...
 *          This parameter can be OPAMP_Selection_OPAMPx where x can be 1 to 4
 *          to select the OPAMP peripheral.
 * @param  OPAMP_InitStruct: pointer to an OPAMP_InitTypeDef structure that
 * contains the configuration information for the specified OPAMP peripheral.
 *           - OPAMP_InvertingInput specifies the inverting input of OPAMP
 *           - OPAMP_NonInvertingInput specifies the non inverting input of
 * OPAMP
 * @retval None
 */
void OPAMP_Init(uint32_t OPAMP_Selection, OPAMP_InitTypeDef *OPAMP_InitStruct) {
  ...
  assert_param(
      IS_OPAMP_INVERTING_INPUT(OPAMP_InitStruct->OPAMP_InvertingInput));
  assert_param(
      IS_OPAMP_NONINVERTING_INPUT(OPAMP_InitStruct->OPAMP_NonInvertingInput));
 ...
 * @param  OPAMP_InitStruct: pointer to an OPAMP_InitTypeDef structure which
 * will be initialized.
 * @retval None
 */
 ...
 * @note   If the selected OPAMP is locked, gain configuration can't be
 ...
 * @note   If the selected OPAMP is locked, Vref configuration can't be
 * performed. To unlock the configuration, perform a system reset.
 * @param  OPAMP_Selection: the selected OPAMP.
 *          This parameter can be OPAMP_Selection_OPAMPx where x can be 1 to 4
 *          to select the OPAMP peripheral.
...
/**
 * @brief  Configure the OPAMP peripheral (secondary inputs) for
 * timer-controlled mux mode according to the specified parameters in
 * OPAMP_InitStruct.
...
void OPAMP_TimerControlledMuxConfig(uint32_t OPAMP_Selection,
  ...
  assert_param(IS_OPAMP_ALL_PERIPH(OPAMP_Selection));
  assert_param(
      IS_OPAMP_SECONDARY_INVINPUT(OPAMP_InitStruct->OPAMP_InvertingInput));
  assert_param(
      IS_OPAMP_NONINVERTING_INPUT(OPAMP_InitStruct->OPAMP_NonInvertingInput));
  ...
  /*!< Clear the secondary inverting bit, secondary non inverting bit and TCMEN
   * bits */
...
/**
 * @brief  Return the output level (high or low) during calibration of the
 * selected OPAMP.
 * @param  OPAMP_Selection: the selected OPAMP.
 *          This parameter can be OPAMP_Selection_OPAMPx where x can be 1 to 4
 *          to select the OPAMP peripheral.
...
uint32_t OPAMP_GetOutputLevel(uint32_t OPAMP_Selection) {
  ...
  if ((*(__IO uint32_t *)(OPAMP_BASE + OPAMP_Selection) & (OPAMP_CSR_OUTCAL)) !=
      0) {
 ...
 *   This parameter can be one of the following values:
 *         @arg OPAMP_Input_Inverting: Inverting input is selected to configure
 * the trimming value
 *         @arg OPAMP_Input_NonInverting: Non inverting input is selected to
 * configure the trimming value
 * @param  OPAMP_TrimValue: the trimming value. This parameter can be any value
 * lower or equal to 0x0000001F.
 * @retval None
 */
