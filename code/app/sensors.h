...
#define VREF 3300
#define AN_RATIO \
    1.22f /* 5000mV/4096 voltage divider ratio is 1.22 (10K/2k=1.5) */
#define VBAT_RATIO                                        \
    13.42f /* 55000mV/4096 voltage divider ratio is 13.42 \
              (100K/10K=11)+(1K/2K=1.5) */
...
uint8_t calculateAFRFromMillivolt(uint16_t afrMin, uint16_t afrMax,
    uint16_t AnVal);
uint16_t calculateFreqWithRatio(uint16_t freq, float ratio);
