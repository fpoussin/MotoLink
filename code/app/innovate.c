...
void readMtsPackets(uint8_t *buf) {
  ...
  // First, check header - packets are big endian so we had to convert them to
  // little endian.
  if ((pkt_header & MTS_HEADER_MASK) == MTS_HEADER_MASK) {
    len_raw =
        ((pkt_header & MTS_LENGTH_HI) >> 1) | (pkt_header & MTS_LENGTH_LO);
    ...
    if (len == 6) {
      ...
      if ((pkt_status_afr_mult & MTS_STATUS_MASK_TEST_LC1) !=
          MTS_STATUS_MASK_LC1) {
        if (dbg_mts)
          chprintf(DBG_STREAM, "->[MTS] Message not from an LC-1/2: %04X\r\n",
                   pkt_status_afr_mult);
        return;
      }
      ...
      if (status == 0) {
        /* Should be 147 */
        // afr_multiplier = ((pkt_status_afr_mult & MTS_AFR_MUL_HI) >> 1) |
        // (pkt_status_afr_mult & MTS_AFR_MUL_LO);
        afr_multiplier = 147;

        /* Sensor value */
        lambda =
            ((pkt_lambda & MTS_LAMBDA_HI) >> 1) | (pkt_lambda & MTS_LAMBDA_LO);
        ...
        if (dbg_mts)
          chprintf(DBG_STREAM, "->[MTS] Sensor warming up\r\n");
      } else if (status & MTS_STATUS_LAMBDA_NEED_CAL) {
        if (dbg_mts)
          chprintf(DBG_STREAM, "->[MTS] Sensor need calibration\r\n");
      } else if (status & MTS_STATUS_LAMBDA_ERROR) {
        if (dbg_mts)
          chprintf(DBG_STREAM, "->[MTS] Sensor error\r\n");
      } else {
        if (dbg_mts)
          chprintf(DBG_STREAM, "->[MTS] Status error: %02x\r\n", status);
      }
    } else {
      if (dbg_mts)
        chprintf(DBG_STREAM, "->[MTS] Length incorrect: %02X, expecting 2\r\n",
                 len);
      return;
    }
  } else {
    if (dbg_mts)
      chprintf(DBG_STREAM, "->[MTS] Not an MTS header: %04X/%04X\r\n",
